...
- `storage/opportunity_store.py` lines 116–121: added `ALTER TABLE outreach_attempts ADD COLUMN warm_lead_score REAL DEFAULT 0.0` migration guard in `_init_db` (try/except `sqlite3.OperationalError`).
- `engine/warm_lead.py` lines 69–70: `enrich_contact_warmth` now assigns `warm_lead_score` as an explicit float clamped to 0–100.
