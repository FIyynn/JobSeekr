...
### Report
- `storage/opportunity_store.py`: line 110 added `warm_lead_score REAL DEFAULT 0.0` to `outreach_attempts` CREATE; lines 116–121 added `ALTER TABLE` migration guard in `_init_db`.
- `engine/warm_lead.py`: no changes — `enrich_contact_warmth` (lines 66–70) already returns `warm_lead_score` as a float 0–100 via `compute_warm_lead_score`.
