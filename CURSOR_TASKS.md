...
### Report
- `storage/opportunity_store.py` lines 110, 116–122: added `warm_lead_score REAL DEFAULT 0.0` to `outreach_attempts` CREATE; added `ALTER TABLE` migration guard in `_init_db`.
- `engine/warm_lead.py`: no changes needed — `enrich_contact_warmth` (lines 66–70) already returns `warm_lead_score` as a float 0–100 via `compute_warm_lead_score`.
