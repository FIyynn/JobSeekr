...
### Report
- `storage/opportunity_store.py`: lines 110 (added `warm_lead_score` column to CREATE), 116–121 (ALTER TABLE migration guard).
- `engine/warm_lead.py`: no changes required — `enrich_contact_warmth` (lines 66–70) already returns `warm_lead_score` as a float 0–100 via `compute_warm_lead_score`.
