...
- Lines 116–121: migration guard `ALTER TABLE outreach_attempts ADD COLUMN warm_lead_score REAL DEFAULT 0.0` (try/except)

**engine/warm_lead.py**
- Line 69: `enrich_contact_warmth` returns `warm_lead_score` as explicit `float` (0–100 via `compute_warm_lead_score`)
