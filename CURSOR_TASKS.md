...
- Lines 128–133: added `ALTER TABLE outreach_attempts ADD COLUMN warm_lead_score REAL DEFAULT 0.0` migration guard in `_init_db` (try/except `sqlite3.OperationalError`)

**engine/warm_lead.py**
- Lines 69–70: `enrich_contact_warmth` now assigns `warm_lead_score` as `float(min(100.0, max(0.0, score)))` (0–100)
