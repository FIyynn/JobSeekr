...
### Report
- `storage/opportunity_store.py` lines 110, 116–121: added `warm_lead_score REAL DEFAULT 0.0` to `outreach_attempts` CREATE; added `ALTER TABLE` migration guard in `_init_db`.
- `engine/warm_lead.py` lines 69–70: `enrich_contact_warmth` now clamps and casts `warm_lead_score` to `float` in 0–100.
