...
- `storage/opportunity_store.py` lines 116-121: added try/except `ALTER TABLE outreach_attempts ADD COLUMN warm_lead_score REAL DEFAULT 0.0` migration guard in `_init_db`.
- `engine/warm_lead.py` line 69: wrapped score assignment with `float()` so `enrich_contact_warmth` returns `warm_lead_score` as a float 0-100.
