...
- `storage/opportunity_store.py` lines 116–121: added `ALTER TABLE outreach_attempts ADD COLUMN warm_lead_score` migration guard (try/except)
- `engine/warm_lead.py` lines 69–70: `enrich_contact_warmth` now sets `warm_lead_score` as `float` clamped 0–100
