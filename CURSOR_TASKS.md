...
### Report
- `storage/opportunity_store.py` lines 110, 115–120: added `warm_lead_score REAL DEFAULT 0.0` to `outreach_attempts` CREATE and ALTER TABLE migration guard in `_init_db`.
- `engine/warm_lead.py`: no changes needed — `enrich_contact_warmth` already returns `warm_lead_score` as a float 0–100.
