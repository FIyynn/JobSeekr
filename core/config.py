...
def _build_compat_extract(app_config: dict[str, Any]) -> dict[str, Any]:
    extract = app_config.get("extract")
    if isinstance(extract, dict):
        return deepcopy(extract)
    return {}


def _build_compat_presets(app_config: dict[str, Any]) -> dict[str, Any]:
    presets = app_config.get("presets")
