...
def _resolve_indexes(count: int, index: Any) -> list[int]:
    if count <= 0:
        return []

    def _normalize_single(value: Any) -> list[int]:
        try:
            numeric = int(value)
        except Exception:
            return []
    ...
    if isinstance(index, range):
        ...
        for chunk in index.split(","):
            ...
            if "-" in piece:
                ...
                values.extend(_normalize_single(piece))
    elif isinstance(index, (list, tuple, set)):
        values = []
        items = list(index)
        if len(items) == 2 and all(str(item).strip().lstrip("-").isdigit() for item in items):
            start = int(items[0])
            end = int(items[1])
            if start < 0 or end < 0:
                return list(range(count))
            ...
            for item in items:
                if int(item) < 0:
                    return list(range(count))
                values.extend(_normalize_single(item))
    else:
        return _normalize_single(index)
...
def extract_listing_detail(
    ...
    targets = _resolve_indexes(len(listings), index)
    result: dict[str, Any] = {
        "dev": {
            "source": "linkedin",
            "index": index,
